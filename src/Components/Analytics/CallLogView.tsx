...
const CallLogView: FunctionComponent<props> = ({callLog, index, onPress}) => {
      ...
      <>
        <TouchableOpacity style={styles.textParent} onPress={onPress}>
          <Text style={styles.head} numberOfLines={2}>
            {callLog.data.customer_name}
          </Text>
          <Text style={styles.subHead}>{callLog.data.stage}</Text>
          <Text style={styles.stage}>{secToTime(callLog.data.duration)}</Text>
        </TouchableOpacity>
        <View style={styles.iconViewFollwUp}>
          <Text style={styles.stage}>{callLog.data.project}</Text>
        </View>
      </>
    </View>
  );

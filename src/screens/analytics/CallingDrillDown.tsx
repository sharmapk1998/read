...
import {getCallLogDrillDownData} from '../../Services/drilldown';
import {arrangeTasks} from '../../Services/tasks';
import {fetchSingleLead} from '../../Services/leads';
import {getCallLogFilterValues} from '../../Services/callLogs';
import CallLogView from '../../Components/Analytics/CallLogView';
...
const CallingDrillDown: FunctionComponent<props> = ({
  navigation,
  route,
  user,
  filters,
  ...
  const callCount = route.params.callCount;
  ...
  useEffect(() => {
    if (user.uid) {
      getCallLogFilterValues(user.uid, dispatcher);
    }
  }, [user.uid]);
  ...
  useEffect(() => {
    if (user.uid) {
      ...
      getCallLogDrillDownData(
        uid ? uid : user.uid,
        1,
        searchString !== '' ? searchString : searchQuery,
        localFilter,
        {[groupFeild]: sort['CALL'] ? 1 : -1},
        role,
        (value) => setLoad(value),
  ...
  const onCallPress = async (leadId: string) => {
...
export default connect(mapStateToProps)(CallingDrillDown);
